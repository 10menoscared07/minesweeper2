...
class PatternBg:
    def __init__(self, resolution=vec2(600,600), scale=1):
        ...
        self.patternid = "Patterns/pattern_01.png"
        ...
        for y in range(int(self.numCells.y)):
            for x in range(int(self.numCells.x)):
                ...
                if numMines != 0 and numMines != "x":
                    ...
                    cell.setValue(numMines, self.images[0])
...
mm = Mainmenu(vec2(640, 640), res)
ms = Minesweeper(vec2(640, 640), res)
...
while 1:
    ...
    # angle += 3.14 * dt
    # ms.setPos(vec2(posx + math.sin(angle)*15, posy))
    ...
    if mm.showDone and not transitionDone:
        mineSweeper = True
